...
class MainActivity: FlutterActivity() {
    // Scanner
    private val COMMAND_CHANNEL = "org.thingsboard.pe.app/command"
    private val SCAN_CHANNEL = "org.thingsboard.pe.app/scan"
    private val PROFILE_INTENT_ACTION = "org.thingsboard.pe.app.SCAN"
        ...
        return object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                if (intent.action.equals(PROFILE_INTENT_ACTION))
                {
                    ...
                    var scanData = intent.getStringExtra(DWInterface.DATAWEDGE_SCAN_EXTRA_DATA_STRING) as String
                    var symbology = intent.getStringExtra(DWInterface.DATAWEDGE_SCAN_EXTRA_LABEL_TYPE) as String
                    var date = Calendar.getInstance().getTime()
                    var df = SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
                    var dateTimeString = df.format(date) as String
                    var currentScan = Scan(scanData);
